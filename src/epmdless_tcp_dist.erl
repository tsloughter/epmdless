                    ...
                    case Driver:connect(Ip, TcpPort, connect_options([{active, false}, {packet, 2}, {reuseaddr, true}, {buffer, 65536}])) of
                        {ok, Socket} ->
                            HSData = #hs_data{
                                ...
                                f_recv = fun Driver:recv/3,
                                f_setopts_pre_nodeup =
                                fun(S) -> inet:setopts (S, [{active, false}, {packet, 4}, nodelay()]) end,
                                ...
                                f_address =
                                fun(_,_) ->
                                    #net_address{address = {Ip,TcpPort}, host = Address, protocol = tcp, family = AddressFamily}
                                end,
                                mf_tick = fun(S) -> ?MODULE:tick(Driver, S) end,
                                mf_getstat = fun ?MODULE:getstat/1,
                                request_type = Type,
